...
//
// Key features of the Sprites exec API used by this package:
// - Sessions persist after client disconnect (no tmux needed)
// - cmd.Wait() blocks until process exits (no polling required)
// - Session IDs enable crash recovery (can reattach to running sessions)
// - Filesystem API for direct file operations (no shell escaping)
// - Network policies for security (whitelist allowed domains)
// - Port notifications for dev servers (auto-expose proxy URLs)
package sprites
