...
func (m *DetailModel) joinTmuxPanes() {
	...
	// Bind F1/F2 to jump to prev/next task while focusing executor pane
	// This allows quick task navigation from any pane (especially useful from Claude pane)
	// Using function keys because they don't produce printable characters in input fields
	...
	exec.CommandContext(ctx, "tmux", "bind-key", "-T", "root", "F1", "run-shell", prevTaskCmd).Run()
	nextTaskCmd := "tmux select-pane -t :.0 && tmux send-keys Down && sleep 0.3 && tmux select-pane -t :.1"
	exec.CommandContext(ctx, "tmux", "bind-key", "-T", "root", "F2", "run-shell", nextTaskCmd).Run()
...
func (m *DetailModel) breakTmuxPanes(saveHeight bool) {
	...
	// Unbind F1/F2 task navigation keybindings
	exec.CommandContext(ctx, "tmux", "unbind-key", "-T", "root", "F1").Run()
	exec.CommandContext(ctx, "tmux", "unbind-key", "-T", "root", "F2").Run()
