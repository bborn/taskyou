...
// TestDistillationCreatesMemories verifies that distillation creates project memories.
func TestDistillationCreatesMemories(t *testing.T) {
	tmpDir := t.TempDir()
	dbPath := filepath.Join(tmpDir, "test.db")
	...
	summary := &TaskSummary{
		...
		Learnings: []Learning{
			{Category: "pattern", Content: "Pattern A works well"},
			{Category: "gotcha", Content: "Watch out for edge case B"},
		},
	}

	// Save memories from the summary
	err = executor.SaveMemoriesFromSummary(task, summary)
	if err != nil {
		t.Fatalf("failed to save memories: %v", err)
	}

	// Verify memories were created
	memories, err := database.GetProjectMemories("test-project", 10)
	if err != nil {
		t.Fatalf("failed to get memories: %v", err)
	}

	// Should have at least 3 memories: 1 decision + 2 learnings
	if len(memories) < 3 {
	...
	for _, m := range memories {
		if m.Category == db.MemoryCategoryDecision && strings.Contains(m.Content, "Used pattern Y") {
			hasDecision = true
			break
		}
	}
	if !hasDecision {
		t.Error("Decision should be saved as a memory")
	}
	...
	hasGotcha := false
	for _, m := range memories {
		if m.Category == db.MemoryCategoryPattern && strings.Contains(m.Content, "Pattern A") {
			hasPattern = true
		}
		if m.Category == db.MemoryCategoryGotcha && strings.Contains(m.Content, "edge case B") {
			hasGotcha = true
		}
	}
	if !hasPattern {
		t.Error("Pattern learning should be saved as a memory")
	}
	if !hasGotcha {
		t.Error("Gotcha learning should be saved as a memory")
	}
}
