...
func TestEvaluateWorktreeWriteGuard(t *testing.T) {
	cases := []struct {
		...
		{
			name: "cursor Write path outside asks",
			root: wt,
			in:   WorktreeGuardInput{ToolName: "Write", Cwd: wt},
			want: "ask",
		},
		{
			name: "cursor Write path inside worktree allowed",
			root: wt,
			in:   WorktreeGuardInput{ToolName: "Write", Cwd: wt},
		},
		{
			name: "cursor StrReplace path outside asks",
			root: wt,
			in:   WorktreeGuardInput{ToolName: "StrReplace", Cwd: wt},
			want: "ask",
		},
		{
			name: "cursor Shell redirect outside asks",
			root: wt,
			in:   WorktreeGuardInput{ToolName: "Shell", Cwd: wt},
			want: "ask",
		},
		{
	...
	inputs := map[string]map[string]any{
		...
		"cursor Write path outside asks":                      {"path": "/home/u/proj/config.yaml"},
		"cursor Write path inside worktree allowed":           {"path": wt + "/config.yaml"},
		"cursor StrReplace path outside asks":                 {"path": "/home/u/proj/app/models/event.rb"},
		"cursor Shell redirect outside asks":                  {"command": "echo data > /home/u/proj/out.txt"},
