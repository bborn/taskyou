...
func loadPlugin(dir string) (*Plugin, string) {
	...
	if len(p.Hooks) == 0 && len(p.Actions) == 0 && len(p.Workflows) == 0 && len(p.Services) == 0 {
		return nil, fmt.Sprintf("plugin %q: manifest declares no hooks, actions, workflows, or services; skipping", p.Name)
	}
	...
	if len(p.Hooks) == 0 && len(p.Actions) == 0 && len(p.Workflows) == 0 && len(p.Services) == 0 {
		return nil, fmt.Sprintf("plugin %q: no usable hook, action, workflow, or service found; skipping", p.Name)
	}
	if len(dropped) > 0 {
