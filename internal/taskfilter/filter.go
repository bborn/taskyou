...
//	@mona                                 the machine a task ran on (OR'd)
//
...
type Query struct {
	...
	// Hosts are the `@host` tokens — the machine a task ran on — OR'd together.
	// A bare "@" is the empty string and means "ran on some other machine".
	Hosts []string
	// LocalHost is this machine's short name, which is what lets `@mona` mean
	// the same thing on mona as anywhere else: a task that ran here carries no
	// placement target at all, so without it the local machine would be the one
	// host in the fleet you could not name. Set it with WithLocalHost.
	LocalHost string
	// Keyword is the free text left after every recognised token was removed.
	Keyword string
}

// hostLocalAliases are the ways to say "not placed anywhere": the resolver
// spells local as an empty target, which is untypeable.
var hostLocalAliases = map[string]bool{
	"local": true, "here": true, "localhost": true,
}

// WithLocalHost names this machine for `@host` matching.
func (q Query) WithLocalHost(name string) Query {
	q.LocalHost = strings.ToLower(strings.TrimSpace(name))
	return q
}
...
func Parse(raw string) Query {
	...
	for _, field := range strings.Fields(rest) {
		// `@host` is its own sigil rather than a prefix:value token, matching
		// the badge already printed on the card.
		if strings.HasPrefix(field, "@") {
			q.Hosts = append(q.Hosts, strings.ToLower(strings.TrimPrefix(field, "@")))
			continue
		}
...
func (q Query) IsZero() bool {
	return !q.HasStructured() && len(q.Projects) == 0 &&
...
func (q Query) HasStructured() bool {
	return len(q.Statuses) > 0 || q.Kind != "" || q.Pinned != nil ||
		q.HasPR != nil || len(q.Tags) > 0 || len(q.Hosts) > 0
}
...
func (q Query) Match(t *db.Task) bool {
	...
	if len(q.Hosts) > 0 && !q.matchHost(t) {
		return false
	}
	return true
}

// matchHost reports whether a task satisfies any one `@host` token.
func (q Query) matchHost(t *db.Task) bool {
	target := strings.ToLower(strings.TrimSpace(t.PlacementTarget))
	for _, host := range q.Hosts {
		switch {
		case host == "":
			// A bare "@" — ran on some other machine at all.
			if target != "" {
				return true
			}
		case hostLocalAliases[host]:
			if target == "" {
				return true
			}
		case target == "":
			// Ran here, so it answers to this machine's name.
			if q.LocalHost != "" && strings.HasPrefix(q.LocalHost, host) {
				return true
			}
		default:
			// Prefix match, so the board narrows as the name is typed and
			// "@mona" still finds "mona.local".
