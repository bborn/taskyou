...
// `@host` moved here from internal/ui so one grammar serves every surface.
// These cases came with it.

func hostTask(id int64, host string) *db.Task {
	return &db.Task{ID: id, Status: db.StatusProcessing, PlacementTarget: host}
}
...
func TestParseHosts(t *testing.T) {
	cases := []struct {
		...
		{"@", []string{""}, ""},
		// Project chips survive untouched; an email address is not a host token.
		{"[offerlab] @mona bump", []string{"mona"}, "[offerlab] bump"},
		{"user@example.com", nil, "user@example.com"},
	}
	for _, c := range cases {
		q := Parse(c.query)
		// Rest(), not Keyword: a project chip is parsed into Projects here
		// rather than left in the free text, and Rest() puts it back — so this
		// still asserts "the host token is the only thing removed".
		if !reflect.DeepEqual(q.Hosts, c.wantHosts) || q.Rest() != c.wantRest {
			t.Errorf("Parse(%q) = (hosts %v, rest %q), want (%v, %q)",
				c.query, q.Hosts, q.Rest(), c.wantHosts, c.wantRest)
		}
	}
}

func TestMatchHost(t *testing.T) {
	all := []*db.Task{hostTask(1, "mona"), hostTask(2, "bruce"), hostTask(3, "mona.local"), hostTask(4, "")}

	ids := func(tasks []*db.Task) string {
		var b []byte
		for _, task := range tasks {
			b = append(b, byte('0'+task.ID))
		}
		return string(b)
	}
	...
	cases := []struct {
		...
		{"no host token is a no-op", "", "spark", "1234"},
		{"one host", "@mona", "spark", "13"},
		{"several hosts are an OR", "@mona @bruce", "spark", "123"},
		{"unknown host matches nothing", "@nowhere", "spark", ""},
		{"local names the tasks that ran here", "@local", "spark", "4"},
		{"here and localhost are the same thing", "@here", "spark", "4"},
		// The local machine is a host like any other: on mona, "@mona" has to
		// find the tasks that ran here, which carry no placement target at all.
		{"this machine answers to its own name", "@mona", "mona", "134"},
		{"bare @ is every placed task", "@", "spark", "123"},
		{"prefix matches while the name is typed", "@mon", "spark", "13"},
	}
	for _, c := range cases {
		q := Parse(c.query).WithLocalHost(c.localHost)
		if got := ids(q.Filter(all)); got != c.want {
			t.Errorf("%s: Parse(%q).Filter() = %q, want %q", c.name, c.query, got, c.want)
		}
	}
}
