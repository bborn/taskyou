...
type Task struct {
	...
	ModelProfile    string // Per-task model profile name (Pi executor): selects provider+model+base_url from the model-profile config. "" = Pi's default model.
	WorktreePath    string
	BranchName      string
...
func (db *DB) CreateTask(t *Task) error {
	...
	result, err := db.Exec(`
		INSERT INTO tasks (title, body, status, type, project, executor, pinned, tags, source_branch, dangerous_mode, permission_mode, remote_control, effort_level, model_profile)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, t.Title, t.Body, t.Status, t.Type, t.Project, t.Executor, t.Pinned, t.Tags, t.SourceBranch, t.DangerousMode, t.PermissionMode, t.RemoteControl, t.EffortLevel, t.ModelProfile)
	if err != nil {
...
func (db *DB) GetTask(id int64) (*Task, error) {
	...
	err := db.QueryRow(`
		SELECT id, title, body, status, type, project, COALESCE(executor, 'claude'),
		       ...
		       COALESCE(source_branch, ''), COALESCE(summary, ''), COALESCE(effort_level, ''), COALESCE(model_profile, ''),
	...
	`, id).Scan(
		...
		&t.SourceBranch, &t.Summary, &t.EffortLevel, &t.ModelProfile,
...
func (db *DB) UpdateTask(t *Task) error {
	...
	_, err := db.Exec(`
		UPDATE tasks SET
			...
			pinned = ?, tags = ?, source_branch = ?, effort_level = ?, model_profile = ?,
	...
	`, t.Title, t.Body, t.Status, t.Type, t.Project, t.Executor,
		...
		t.Pinned, t.Tags, t.SourceBranch, t.EffortLevel, t.ModelProfile, t.ID)
	if err != nil {
