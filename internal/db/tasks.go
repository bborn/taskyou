...
func (db *DB) CreateProject(p *Project) error {
	...
	result, err := db.Exec(`
		INSERT INTO projects (name, path, aliases, instructions, actions, color, claude_config_dir, sprite_name, sprite_status)
		VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
	`, p.Name, p.Path, p.Aliases, p.Instructions, string(actionsJSON), p.Color, p.ClaudeConfigDir, p.SpriteName, p.SpriteStatus)
	if err != nil {
...
func (db *DB) UpdateProject(p *Project) error {
	...
	_, err := db.Exec(`
		...
		color = ?, claude_config_dir = ?, sprite_name = ?, sprite_status = ?
		WHERE id = ?
	`, p.Name, p.Path, p.Aliases, p.Instructions, string(actionsJSON), p.Color, p.ClaudeConfigDir, p.SpriteName, p.SpriteStatus, p.ID)
	if err != nil {
...
func (db *DB) ListProjects() ([]*Project, error) {
	rows, err := db.Query(`
		SELECT id, name, path, aliases, instructions, COALESCE(actions, '[]'),
		       COALESCE(color, ''), COALESCE(claude_config_dir, ''),
	...
	for rows.Next() {
		...
		if err := rows.Scan(&p.ID, &p.Name, &p.Path, &p.Aliases, &p.Instructions, &actionsJSON, &p.Color, &p.ClaudeConfigDir, &p.SpriteName, &p.SpriteStatus, &p.CreatedAt); err != nil {
...
func (db *DB) GetProjectByName(name string) (*Project, error) {
	...
	`, name).Scan(&p.ID, &p.Name, &p.Path, &p.Aliases, &p.Instructions, &actionsJSON, &p.Color, &p.ClaudeConfigDir, &p.SpriteName, &p.SpriteStatus, &p.CreatedAt)
	if err == nil {
		json.Unmarshal([]byte(actionsJSON), &p.Actions)
		return p, nil
	}
