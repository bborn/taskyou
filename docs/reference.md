...
[Start with your first task](getting-started.md) or [choose a workflow](workflows.md). This manual covers the full command surface and configuration.
...
### Workflow handoff
[![Completed plan with implementation already processing](media/workflow-handoff.png)](media/workflow-handoff.png)
...
[![CLI](screenshots/cli-board.png)](screenshots/cli-board.png)
...
[![Web UI](screenshots/gui-board.png)](screenshots/gui-board.png)
...
The same UI is also served in your browser at `http://localhost:8080` whenever `ty serve` runs with the embedded UI (`make build-ui build`). Desktop gets a real PTY executor terminal; the browser falls back to a live terminal mirror. Source lives in [`desktop/`](../desktop/).
...
See [the agent orchestration guide](orchestrator.md) for using the CLI from an agent.
...
Install Go 1.26.0 or later from [go.dev/dl](https://go.dev/dl/).
...
See [docs/orchestrator.md](orchestrator.md) for a complete guide to building your own orchestration agent.
...
> Developers who want to add another backend should read [`docs/executor_interface.md`](executor_interface.md) for the full `TaskExecutor` contract.
...
See [docs/plugins.md](plugins.md#taskplacement--the-one-hook-ty-asks-a-question-of)
...
browser, and with `@` in the TUI. See [remote execution](remote-execution.md)
...
contribute, [docs/plugins.md](plugins.md) for the manifest format,
[`examples/plugins/`](../examples/plugins/) for more (desktop-notify, slack, worktree),
and [docs/plugin-ideas.md](plugin-ideas.md) for ideas.
...
[![Annotating a page with a region and comment](screenshots/browser-annotation.png)](screenshots/browser-annotation.png)
