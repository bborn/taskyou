...
# A TRACKED file that init rewrites, exactly like Rails' db/structure.sql after migrate.
mkdir -p "$PROJECT_PATH/db"
echo "-- schema v1" > "$PROJECT_PATH/db/structure.sql"
# This is the whole point: worktree setup that (a) outlasts a sweep tick and (b) leaves a
# tracked file dirty, so "worktree is clean" is never true — as in every real project.
cat > "$PROJECT_PATH/bin/worktree-setup" <<EOF
#!/usr/bin/env bash
echo "worktree-setup: simulating clone + bundle + migrate (${INIT_DELAY}s)"
sleep ${INIT_DELAY}
echo "-- regenerated by migrate at \$(date +%s)" >> db/structure.sql
echo "worktree-setup: done (db/structure.sql now dirty, as in a real Rails app)"
EOF
...
name: stress
description: Slow-init + dirty-init + concurrency regression workflow. Trivial work; the point is the DAG.
steps:
  - name: Root
    model: haiku
