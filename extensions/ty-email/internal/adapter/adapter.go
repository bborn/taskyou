...
type Email struct {
	...
	AutoReply   bool         // True if the email is auto-generated (vacation responder, bounce, our own outbound)
}
...
// loopHeaderValue marks outbound mail so ty-email never re-processes its own messages.
const loopHeaderValue = "true"

// LoopHeader is set on every outbound email and checked on inbound.
const LoopHeader = "X-TY-Email"

// isAutoGenerated reports whether an email is machine-generated based on its
// headers (vacation responders, bounces, mailing lists, or ty-email itself).
// get returns the value of a header by name (empty string if absent).
func isAutoGenerated(get func(string) string) bool {
	if v := strings.ToLower(strings.TrimSpace(get("Auto-Submitted"))); v != "" && v != "no" {
		return true
	}
	switch strings.ToLower(strings.TrimSpace(get("Precedence"))) {
	case "bulk", "junk", "auto_reply":
		return true
	}
	if get("X-Autoreply") != "" || get("X-Autorespond") != "" {
		return true
	}
	if get(LoopHeader) != "" {
		return true
